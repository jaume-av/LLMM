...
**Nota:** Els enllaços de navegació que no són essencials, com un índex d'una secció o enllaços interns, no haurien d'anar dins d'un `<nav>`. És millor utilitzar una etiqueta `<aside>` o `<footer>`.
...
**Nota:** Recorda que `<aside>` no substitueix un `<section>` o `<div>` si el contingut és central per a la pàgina.
