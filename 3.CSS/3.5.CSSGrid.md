...
### 4. **Grid Tracks**
Els **Grid Tracks** són els espais que hi ha entre dues **Grid Lines** consecutives. Aquests espais representen les **files** i **columnes** del grid.

- **Files** es defineixen amb `grid-template-rows`.
- **Columnes** es defineixen amb `grid-template-columns`.

#### Exemple:
```css
.container {
    display: grid;
    grid-template-columns: 1fr 2fr; /* Una columna amb 1 fracció i una altra amb 2 fraccions */\n    grid-template-rows: 100px auto; /* Una fila fixa de 100px i una fila automàtica */\n}
```
...
- La segona fila s'ajusta automàticament al contingut.
...
Permet assignar noms a les àrees de la graella i organitzar els elements.
...
CSS Grid és una eina increïblement potent i versàtil per crear **layouts complexos i responsius**. Amb les propietats explicades i exemples pràctics, ara tens una base sòlida per començar a aplicar CSS Grid als teus projectes.
