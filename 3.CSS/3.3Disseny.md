...
### `display` — Com es mostra un element**

La propietat **`display`** defineix el comportament bàsic d’un element en el layout:
si trenca línia, si pot tindre `width` i `height`, si es comporta com un contenidor, etc.

| Valor            | Trenca línia     | Accepta width/height | Ocupa tota l’amplada | Situació típica              |
| ---------------- | ---------------- | -------------------- | -------------------- | ---------------------------- |
| **block**        | Sí               | Sí                   | Sí                   | `<div>`, `<p>`, `<h1>`       |
| **inline**       | No               | No                   | No                   | `<span>`, `<a>`              |
| **inline-block** | No               | Sí                   | No                   | Botons, targetes petites     |
| **none**         | —                | —                    | —                    | Ocultar totalment un element |
| **flex**         | Depén dels fills | Sí                   | No necessàriament    | Layouts flexibles moderns    |
| **grid**         | Depén dels fills | Sí                   | No necessàriament    | Layouts en files i columnes  |

---

### **`position` — Com es posiciona un element**

La propietat **`position`** controla **com i respecte a què** es col·loca un element:
si ocupa espai, si es desplaça, i quin és el seu punt de referència.

| Valor        | Ocupa espai en el flux | Es pot moure amb top/left… | Referència de posició                     | Ús principal              |
| ------------ | ---------------------- | -------------------------- | ----------------------------------------- | ------------------------- |
| **static**   | Sí                     | No                         | Sense especial                            | Valor per defecte         |
| **relative** | Sí                     | Sí                         | La seua posició original                  | Ajusts lleugers           |
| **absolute** | No                     | Sí                         | Primer pare amb `position`                | Pop-ups, etiquetes        |
| **fixed**    | No                     | Sí                         | Finestra del navegador                    | Menús fixes, banners      |
| **sticky**   | Sí                     | Sí                         | Es mou fins a un punt → després queda fix | Capçaleres que s’enganxen |

---

### **`float` i `clear` — Comportament flotant**

Les propietats **`float`** i **`clear`** provenen del CSS clàssic i permeten fer que elements floten a un costat i que el text o altres elements s’hi adaptaren al voltant.

| Propietat                      | Significat                                                 | Exemple        | Ús habitual                               |
| ------------------------------ | ---------------------------------------------------------- | -------------- | ----------------------------------------- |
| **float: left / right**        | L’element flota cap a un costat i el text s’hi ajusta      | `float: left;` | Imatges dins d’articles                   |
| **clear: left / right / both** | Evita que l’element se situe al costat d’elements flotants | `clear: both;` | Tancar zones flotants, evitar solapaments |

---

### **`visibility` — Mostrar o ocultar mantenint l’espai**

La propietat **`visibility`** determina si l’element és visible o no, però a diferència de `display: none`, **manté l’espai reservat**.

| Valor       | Es mostra? | Ocupa espai? | Diferència amb `display: none`      |
| ----------- | ---------- | ------------ | ----------------------------------- |
| **visible** | Sí         | Sí           | —                                   |
| **hidden**  | No         | **Sí**       | Manté el lloc encara que no es veja |
