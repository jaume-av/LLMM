...
### 3. Afegir una funció nova en `spa.js`
...
### 4. Crear la vista del formulari
...
### 5. Crear l’app Alpine del formulari
...
Ara bé, esta versió pot donar problemes si algun registre no té imatge, o si el camp `IMATGE` està buit o val `null`.

Per això, és millor usar una versió més segura:
...
* si `ciutat.IMATGE` té valor, es comprova si és URL o imatge local
* si `ciutat.IMATGE` està buit, es mostra una imatge per defecte
* la imatge per defecte hauria d’estar en:

```text
public/images/ciutats/default.jpg
```
...
<td>
    <img
        :src="pais.IMATGE
            ? (pais.IMATGE.startsWith('http')
                ? pais.IMATGE
                : 'images/paisos/' + pais.IMATGE)
            : 'images/paisos/default.jpg'"
        width="100">
</td>
```
...
<td>
    <img
        :src="provincia.IMATGE
            ? (provincia.IMATGE.startsWith('http')
                ? provincia.IMATGE
                : 'images/provincies/' + provincia.IMATGE)
            : 'images/provincies/default.jpg'"
        width="100">
</td>
```

Per tant, la part que canvia és només la carpeta:
...
* Si tenim varies carpetes per a una mateixa vista, la forma més neta és guardar la carpeta dins de la base de dades:
...
```html
<img src="images/ciutats/costera/castello.jpg">
```

Així podem tindre diverses subcarpetes dins de `images/ciutats/` sense haver de complicar més el codi. Només hem de guardar en la base de dades la ruta relativa a partir de la carpeta de ciutats.
