...
## Paràgrafs `<p>` i salts de línia `<br>`
...
- **GET**: Sol·licita dades d'un recurs específic.
- **POST**: Envia dades per a ser processades.
- **PUT**: Actualitza o crea un recurs.
- **DELETE**: Elimina un recurs específic al servidor.

### Formularis – Paràmetres de `<input>`
L'element `<input>` defineix un punt d'entrada de dades, sovint acompanyat per una etiqueta `<label>`. Atributs comuns de `<input>`:
- **type**: Indica el tipus de control ("text", "button", "radio", etc.).
- **name**: Nom del camp per identificar les dades.
- **value**: Estableix el valor inicial.
...
**`<textarea>`** s'utilitza per a àrees de text grans.
...
Aquestes etiquetes i atributs permeten crear formularis personalitzats i funcionals per millorar l'experiència d'usuari i la recollida de dades.
